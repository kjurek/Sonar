...
#define THROW_EXCEPTION(msg) throw ProcessManagerException(msg, std::string(__FILE__) + std::string(":") + std::to_string(__LINE__))
...
public:
    ...
    void write_memory(DWORD address, T value) const
	{
        ...
        BOOL operationResult = WriteProcessMemory(_process_handle, reinterpret_cast<LPVOID>(address), &value, sizeof(T), NULL);
    ...
    void write_memory(DWORD address, T* values, size_t count) const
	{
        ...
        BOOL operationResult = WriteProcessMemory(_process_handle, address, values, count * sizeof(T), NULL);
		if (FALSE == operationResult)
		{
            THROW_EXCEPTION("WriteProcessMemory failed");
		}
    ...
    T read_memory(DWORD address) const
	{
        validate_address(address, sizeof(T));
        T result;
        BOOL operationResult = ReadProcessMemory(_process_handle, reinterpret_cast<LPCVOID>(address), &result, sizeof(T), NULL);
		if (FALSE == operationResult)
		{
    ...
    std::vector<T> read_memory(DWORD address, size_t count) const
	{
        ...
        BOOL operationResult = ReadProcessMemory(_process_handle, reinterpret_cast<LPCVOID>(address), &result[0], count * sizeof(T), NULL);
		if (FALSE == operationResult)
		{
            THROW_EXCEPTION("ReadProcessMemory failed");
		}
