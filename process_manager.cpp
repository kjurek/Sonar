...
ProcessManager::ProcessManager() : _window_handle(0), _pid(0), _process_handle(0) {}
...
{
    return (0 != _pid) && (0 != _window_handle) && (0 != _process_handle);
}
...
std::vector<std::pair<std::string, HMODULE>> ProcessManager::get_modules() const
{
    ...
    BOOL operation_result = EnumProcessModules(_process_handle,
                                               modules,
                                               sizeof(modules),
                                               &cb_needed);
    ...
    for (unsigned int i = 0; i < (cb_needed / sizeof(HMODULE)); i++) {
        ...
        operation_result = GetModuleFileNameEx(_process_handle,
                                               modules[i],
                                               module_name_tchar,
                                               sizeof(module_name_tchar) / sizeof(TCHAR));
        if (FALSE == operation_result) {
...
void ProcessManager::validate_address(DWORD address, size_t nbytes) const
{
    ...
    BOOL operation_result = VirtualQueryEx(_process_handle,
                                           (void *) address,
                                           &mbi,
