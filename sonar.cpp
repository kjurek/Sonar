...
bool Sonar::is_in_game() const
{
   ...
   DWORD client_state_address = _manager.read_memory<DWORD>(get_engine_address() + hazedumper::signatures::dwClientState);
   DWORD client_state = _manager.read_memory<DWORD>(client_state_address + hazedumper::signatures::dwClientState_State);
   return client_state;
}

int Sonar::detect_enemies() const
{
    DWORD local_player_address = _manager.read_memory<DWORD>(get_client_address() + hazedumper::signatures::dwLocalPlayer);
